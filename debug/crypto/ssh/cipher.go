...
func newGCMCipher(key, iv, unusedMacKey []byte, unusedAlgs directionAlgorithms) (packetCipher, error) {
	...
	log.Printf("newGCMCipher key is %x, iv is %x\n", key, iv)
	// log.Printf("directionAlgorithms is %s, %s, %s\n", unusedAlgs.Cipher, unusedAlgs.MAC, unusedAlgs.Compression)
	c, err := aes.NewCipher(key)
	if err != nil {
		return nil, err
	}
...
func (c *gcmCipher) readCipherPacket(seqNum uint32, r io.Reader) ([]byte, error) {
	...
	c.incIV()
	log.Printf("readCipherPacket. plain is %x\n", plain)
...
func (c *chacha20Poly1305Cipher) readCipherPacket(seqNum uint32, r io.Reader) ([]byte, error) {
	...
	log.SetFlags(log.Lshortfile)
	log.Printf("readCipherPacket key is %x, nonce is %x\n", c.contentKey[:], nonce)
